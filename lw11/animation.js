window.addEventListener('DOMContentLoaded', () => {
    ...
    submit.addEventListener("click", () => {
        ...
        if (validation(input)) {
            ...
            let agreement = "Согласия на рассылку нет";
            if (checkbox[0].checked) {
                agreement = "Согласие на рассылку есть";
            }
            let activity = "Деятельность не выбрана";
    ...
    function addClass(element) {
        element.classList.add("form__text-place_another-border");
        element.addEventListener("click", () => {
            for (const i of element.classList) {
                if (i === "form__text-place_another-border") {
                    element.classList.remove("form__text-place_another-border");
                }
            }
        });
    }
